...
esp_err_t mcp23017_set_pullup(mcp23017_handle_t dev, uint16_t pins)
{
    ...
    return mcp23017_write(dev, MCP23017_REG_GPPUA, sizeof(data), data); //set REG_GPIOA(); REG_GPIOB();
}
...
esp_err_t mcp23017_read_io(mcp23017_handle_t dev, mcp23017_gpio_port_t gpio, uint8_t *data)
{
    MCP23017_CHECK(dev != NULL, "invalid arg", 0)
    mcp23017_dev_t *p_device = (mcp23017_dev_t *) dev;

    return i2c_bus_read_byte(p_device->i2c_dev,
                      (gpio == MCP23017_GPIOA) ? MCP23017_REG_GPIOA : MCP23017_REG_GPIOB,
                      data);
}
