...
#define ESP_ERROR_CHECK(x)                   \
  do {                                       \
    esp_err_t rc = (x);                      \
    if (rc != ESP_OK) {                      \
      VA_LOGE("err", "esp_err_t = %d", rc); \
      assert(0 && #x);                       \
    }                                        \
  } while (0);

static const char *get_string_for_msg(uint8_t msg) {
  switch (msg) {
    case U8X8_MSG_GPIO_AND_DELAY_INIT:
      return "U8X8_MSG_GPIO_AND_DELAY_INIT";
    case U8X8_MSG_DELAY_MILLI:
      return "U8X8_MSG_DELAY_MILLI";
    case U8X8_MSG_DELAY_10MICRO:
      return "U8X8_MSG_DELAY_10MICRO";
    case U8X8_MSG_DELAY_100NANO:
      return "U8X8_MSG_DELAY_100NANO";
    case U8X8_MSG_CAD_SEND_DATA:
      return "U8X8_MSG_CAD_SEND_DATA";
    case U8X8_MSG_DELAY_NANO:
      return "U8X8_MSG_DELAY_NANO";
    default:
      return "";
  }
}
...
uint8_t u8g2_esp32_spi_byte_cb(u8x8_t* u8x8,
                               ...
                               uint8_t arg_int,
                               void* arg_ptr) {
//  VA_LOGD(TAG, "spi_byte_cb: Received a msg: %d (%s), arg_int: %d, arg_ptr: %p",
  //         msg, get_string_for_msg(msg), arg_int, arg_ptr);
  switch (msg) {
    ...
    case U8X8_MSG_BYTE_SEND: {
      ...
      // VA_LOGI(TAG, "... Transmitting %d bytes.", arg_int);
...
/*uint8_t u8g2_esp32_i2c_byte_cb(u8x8_t* u8x8,
                               ...
                               uint8_t arg_int,
                               void* arg_ptr) {
  VA_LOGD(TAG, "i2c_cb: Received a msg: %d, arg_int: %d, arg_ptr: %p", msg,
           arg_int, arg_ptr);
  ...
  switch (msg) {
    ...
    case U8X8_MSG_BYTE_INIT: {
      ...
      VA_LOGI(TAG, "sda_io_num %d", u8g2_esp32_hal.bus.i2c.sda);
      conf.sda_io_num = u8g2_esp32_hal.bus.i2c.sda;
      conf.sda_pullup_en = GPIO_PULLUP_ENABLE;
      VA_LOGI(TAG, "scl_io_num %d", u8g2_esp32_hal.bus.i2c.scl);
      conf.scl_io_num = u8g2_esp32_hal.bus.i2c.scl;
      conf.scl_pullup_en = GPIO_PULLUP_ENABLE;
      VA_LOGI(TAG, "clk_speed %d", I2C_MASTER_FREQ_HZ);
      conf.master.clk_speed = I2C_MASTER_FREQ_HZ;
      VA_LOGI(TAG, "i2c_param_config %d", conf.mode);
      ESP_ERROR_CHECK(i2c_param_config(I2C_MASTER_NUM, &conf));
      VA_LOGI(TAG, "i2c_driver_install %d", I2C_MASTER_NUM);
    ...
    case U8X8_MSG_BYTE_START_TRANSFER: {
      ...
      VA_LOGD(TAG, "Start I2C transfer to %02X.", i2c_address >> 1);
...
uint8_t u8g2_esp32_gpio_and_delay_cb(u8x8_t* u8x8,
                                     ...
                                     void* arg_ptr) {
//  VA_LOGD(TAG,
//           "gpio_and_delay_cb: Received a msg: %d (%s), arg_int: %d, arg_ptr: %p",
//           msg, get_string_for_msg(msg), arg_int, arg_ptr);
