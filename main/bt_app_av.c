...
static uint8_t connected_remote_bda[ESP_BD_ADDR_LEN] = { 0 };
...
static void bt_av_notify_evt_handler(uint8_t event_id, esp_avrc_rn_param_t *event_parameter)
{
    ...
    case ESP_AVRC_RN_PLAY_STATUS_CHANGE:
        VA_LOGI(BT_AV_TAG, "Playback status changed: 0x%x", event_parameter->playback);
        bt_av_playback_changed();
        break;
    /* when track playing position changed, this event comes */
    case ESP_AVRC_RN_PLAY_POS_CHANGED: {
        VA_LOGI(BT_AV_TAG, "Play position changed: %"PRIu32"-ms", event_parameter->play_pos);
        bt_av_play_pos_changed();
        va_msg_post_i(VA_MSG_AUDIO_PLAY_POS_CHANGED, event_parameter->play_pos);
        break;
    }
    /* others */
    default:
        VA_LOGW(BT_AV_TAG, "unhandled event: %d", event_id);
        break;
    }
}
...
static void bt_av_hdl_a2d_evt(uint16_t event, void *p_param)
{
    ...
    case ESP_A2D_CONNECTION_STATE_EVT: {
        ...
        VA_LOGD(BT_AV_TAG, "A2DP connection state: %s, [%02x:%02x:%02x:%02x:%02x:%02x]",
            s_a2d_conn_state_str[a2d->conn_stat.state], bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
        
        enum va_audio_connection_state connection_state = ESP_A2D_CONNECTION_STATE_DISCONNECTED;
        switch (a2d->conn_stat.state) {
            case ESP_A2D_CONNECTION_STATE_DISCONNECTED:
                connection_state = VA_AUDIO_CONNECTION_STATE_DISCONNECTED;
                esp_bt_gap_set_scan_mode(ESP_BT_CONNECTABLE, ESP_BT_NON_DISCOVERABLE);
                memset(connected_remote_bda, 0, ESP_BD_ADDR_LEN);
                break;
            case ESP_A2D_CONNECTION_STATE_CONNECTED:
                connection_state = VA_AUDIO_CONNECTION_STATE_CONNECTED;
                esp_bt_gap_set_scan_mode(ESP_BT_NON_CONNECTABLE, ESP_BT_NON_DISCOVERABLE);
                memcpy(connected_remote_bda, bda, ESP_BD_ADDR_LEN);
                va_audio_add_bonded_device(bda);
                esp_bt_gap_read_remote_name(bda);
                break;
            case ESP_A2D_CONNECTION_STATE_CONNECTING:
                connection_state = VA_AUDIO_CONNECTION_STATE_CONNECTING;
                break;
            case ESP_A2D_CONNECTION_STATE_DISCONNECTING:
                connection_state = VA_AUDIO_CONNECTION_STATE_DISCONNECTING;
                break;
        }
    ...
    case ESP_A2D_AUDIO_STATE_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        VA_LOGD(BT_AV_TAG, "A2DP audio state: %s", s_a2d_audio_state_str[a2d->audio_stat.state]);
        ...
        switch (s_audio_state) {
            ...
            default:
                VA_LOGE(BT_AV_TAG, "Unhandled state %d", s_audio_state);
                break;
        }
        break;
    }
    ...
    case ESP_A2D_AUDIO_CFG_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        VA_LOGI(BT_AV_TAG, "A2DP audio stream configuration, codec type: %d", a2d->audio_cfg.mcc.type);
        ...
        if (a2d->audio_cfg.mcc.type == ESP_A2D_MCT_SBC) {
            ...
            i2s_std_slot_config_t slot_cfg = I2S_STD_PHILIPS_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_32BIT, ch_count);
            i2s_channel_reconfig_std_clock(tx_chan, &clk_cfg);
            i2s_channel_reconfig_std_slot(tx_chan, &slot_cfg);
            i2s_channel_enable(tx_chan);
            VA_LOGD(BT_AV_TAG, "Configure audio player: %x-%x-%x-%x",
                     a2d->audio_cfg.mcc.cie.sbc[0],
                     a2d->audio_cfg.mcc.cie.sbc[1],
                     a2d->audio_cfg.mcc.cie.sbc[2],
                     a2d->audio_cfg.mcc.cie.sbc[3]);
            VA_LOGI(BT_AV_TAG, "Audio player configured, sample rate: %d", sample_rate);
        }
        break;
    }
    ...
    case ESP_A2D_PROF_STATE_EVT: {
        ...
        if (ESP_A2D_INIT_SUCCESS == a2d->a2d_prof_stat.init_state) {
            VA_LOGI(BT_AV_TAG, "A2DP PROF STATE: Init Complete");
        } else {
            VA_LOGI(BT_AV_TAG, "A2DP PROF STATE: Deinit Complete");
        }
        break;
    }
    ...
    case ESP_A2D_SNK_PSC_CFG_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        VA_LOGI(BT_AV_TAG, "protocol service capabilities configured: 0x%x ", a2d->a2d_psc_cfg_stat.psc_mask);
        if (a2d->a2d_psc_cfg_stat.psc_mask & ESP_A2D_PSC_DELAY_RPT) {
            VA_LOGI(BT_AV_TAG, "Peer device support delay reporting");
        } else {
            VA_LOGI(BT_AV_TAG, "Peer device unsupported delay reporting");
        }
        break;
    }
    ...
    case ESP_A2D_SNK_SET_DELAY_VALUE_EVT: {
        ...
        if (ESP_A2D_SET_INVALID_PARAMS == a2d->a2d_set_delay_value_stat.set_state) {
            VA_LOGI(BT_AV_TAG, "Set delay report value: fail");
        } else {
            VA_LOGI(BT_AV_TAG, "Set delay report value: success, delay_value: %u * 1/10 ms", a2d->a2d_set_delay_value_stat.delay_value);
        }
        break;
    }
    ...
    case ESP_A2D_SNK_GET_DELAY_VALUE_EVT: {
        a2d = (esp_a2d_cb_param_t *)(p_param);
        VA_LOGI(BT_AV_TAG, "Get delay report value: delay_value: %u * 1/10 ms", a2d->a2d_get_delay_value_stat.delay_value);
...
static void bt_av_hdl_avrc_ct_evt(uint16_t event, void *p_param)
{
    VA_LOGD(BT_RC_CT_TAG, "%s event: %d", __func__, event);
    ...
    case ESP_AVRC_CT_CONNECTION_STATE_EVT: {
        ...
        VA_LOGI(BT_RC_CT_TAG, "AVRC conn_state event: state %d, [%02x:%02x:%02x:%02x:%02x:%02x]",
    ...
    case ESP_AVRC_CT_PASSTHROUGH_RSP_EVT: {
        VA_LOGI(BT_RC_CT_TAG, "AVRC passthrough rsp: key_code 0x%x, key_state %d, rsp_code %d", rc->psth_rsp.key_code,
                    rc->psth_rsp.key_state, rc->psth_rsp.rsp_code);
        break;
    }
    /* when metadata response, this event comes */
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        VA_LOGI(BT_RC_CT_TAG, "AVRC metadata rsp: attribute id 0x%x, %s", rc->meta_rsp.attr_id, rc->meta_rsp.attr_text);
        switch (rc->meta_rsp.attr_id) {
            case ESP_AVRC_MD_ATTR_TITLE:
                va_msg_post_ptr(
                    VA_MSG_AUDIO_TITLE_CHANGED,
                    strdup((const char *)rc->meta_rsp.attr_text));
                break;
    ...
    case ESP_AVRC_CT_CHANGE_NOTIFY_EVT: {
        VA_LOGI(BT_RC_CT_TAG, "AVRC event notification: %d", rc->change_ntf.event_id);
        bt_av_notify_evt_handler(rc->change_ntf.event_id, &rc->change_ntf.event_parameter);
        break;
    }
    /* when feature of remote device indicated, this event comes */
    case ESP_AVRC_CT_REMOTE_FEATURES_EVT: {
        VA_LOGI(BT_RC_CT_TAG, "AVRC remote features %"PRIx32", TG features %x", rc->rmt_feats.feat_mask, rc->rmt_feats.tg_feat_flag);
#if CONFIG_EXAMPLE_AVRCP_CT_COVER_ART_ENABLE
        if ((rc->rmt_feats.tg_feat_flag & ESP_AVRC_FEAT_FLAG_TG_COVER_ART) && !cover_art_connected) {
            VA_LOGW(BT_RC_CT_TAG, "Peer support Cover Art feature, start connection...");
    ...
    case ESP_AVRC_CT_GET_RN_CAPABILITIES_RSP_EVT: {
        VA_LOGI(BT_RC_CT_TAG, "remote rn_cap: count %d, bitmask 0x%x", rc->get_rn_caps_rsp.cap_count,
    ...
    case ESP_AVRC_CT_COVER_ART_STATE_EVT: {
        if (rc->cover_art_state.state == ESP_AVRC_COVER_ART_CONNECTED) {
            ...
            VA_LOGW(BT_RC_CT_TAG, "Cover Art Client connected");
        }
        else {
            cover_art_connected = false;
            VA_LOGW(BT_RC_CT_TAG, "Cover Art Client disconnected, reason:%d", rc->cover_art_state.reason);
        }
        break;
    }
    case ESP_AVRC_CT_COVER_ART_DATA_EVT: {
        ...
        if (rc->cover_art_data.final) {
            if(rc->cover_art_data.status == ESP_BT_STATUS_SUCCESS) {
                VA_LOGI(BT_RC_CT_TAG, "Cover Art Client final data event, image size: %lu bytes", cover_art_image_size);
            }
            else {
                VA_LOGE(BT_RC_CT_TAG, "Cover Art Client get operation failed");
            }
    ...
    /* others */
    default:
        VA_LOGE(BT_RC_CT_TAG, "%s unhandled event: %d", __func__, event);
        break;
    }
}
...
static void bt_av_hdl_avrc_tg_evt(uint16_t event, void *p_param)
{
    VA_LOGD(BT_RC_TG_TAG, "%s event: %d", __func__, event);
    ...
    case ESP_AVRC_TG_CONNECTION_STATE_EVT: {
        uint8_t *bda = rc->conn_stat.remote_bda;
        VA_LOGI(BT_RC_TG_TAG, "AVRC conn_state evt: state %d, [%02x:%02x:%02x:%02x:%02x:%02x]",
                 rc->conn_stat.connected, bda[0], bda[1], bda[2], bda[3], bda[4], bda[5]);
        break;
    }
    /* when passthrough commanded, this event comes */
    case ESP_AVRC_TG_PASSTHROUGH_CMD_EVT: {
        VA_LOGI(BT_RC_TG_TAG, "AVRC passthrough cmd: key_code 0x%x, key_state %d", rc->psth_cmd.key_code, rc->psth_cmd.key_state);
        break;
    }
    /* when absolute volume command from remote device set, this event comes */
    case ESP_AVRC_TG_SET_ABSOLUTE_VOLUME_CMD_EVT: {
        VA_LOGI(BT_RC_TG_TAG, "AVRC set absolute volume: %d%%", (int)rc->set_abs_vol.volume * 100 / 0x7f);
        break;
    }
    /* when notification registered, this event comes */
    case ESP_AVRC_TG_REGISTER_NOTIFICATION_EVT: {
        VA_LOGI(BT_RC_TG_TAG, "AVRC register event notification: %d, param: 0x%"PRIx32, rc->reg_ntf.event_id, rc->reg_ntf.event_parameter);
        if (rc->reg_ntf.event_id == ESP_AVRC_RN_VOLUME_CHANGE) {
            esp_avrc_rn_param_t rn_param;
            rn_param.volume = 0x7F;
    ...
    case ESP_AVRC_TG_REMOTE_FEATURES_EVT: {
        VA_LOGI(BT_RC_TG_TAG, "AVRC remote features: %"PRIx32", CT features: %x", rc->rmt_feats.feat_mask, rc->rmt_feats.ct_feat_flag);
        break;
    }
    /* others */
    default:
        VA_LOGE(BT_RC_TG_TAG, "%s unhandled event: %d", __func__, event);
        break;
    }
}
...
void bt_app_a2d_cb(esp_a2d_cb_event_t event, esp_a2d_cb_param_t *param)
{
    ...
    default:
        VA_LOGE(BT_AV_TAG, "Invalid A2DP event: %d", event);
        break;
    }
}

static int32_t buf_nbr_samples = 0;
static int32_t *buf = NULL;

void IRAM_ATTR bt_app_a2d_data_cb(const uint8_t *data, uint32_t len)
{
    const int nbr_samples = len / sizeof(int16_t);

    if (buf_nbr_samples < nbr_samples) {
        if (buf) {
            free(buf);
        }
        buf = malloc(sizeof(int32_t) * nbr_samples);
        buf_nbr_samples = nbr_samples;
    }

    const int16_t *src = (const int16_t *)data;

    va_audio_submit_samples_for_waveform(src, nbr_samples);
    va_audio_apply_volume(src, nbr_samples, buf);

    write_ringbuf((const uint8_t *)buf, nbr_samples * sizeof(int32_t));
}
...
void bt_app_rc_ct_cb(esp_avrc_ct_cb_event_t event, esp_avrc_ct_cb_param_t *param)
{
    ...
    case ESP_AVRC_CT_COVER_ART_DATA_EVT:
        ...
        break;
    default:
        VA_LOGE(BT_RC_CT_TAG, "Invalid AVRC event: %d", event);
        break;
    }
}
...
void bt_app_rc_tg_cb(esp_avrc_tg_cb_event_t event, esp_avrc_tg_cb_param_t *param)
{
    ...
    case ESP_AVRC_TG_SET_PLAYER_APP_VALUE_EVT:
        ...
        break;
    default:
        VA_LOGE(BT_RC_TG_TAG, "Invalid AVRC event: %d", event);
        break;
    }
}

void bt_app_disconnect(void) {
    esp_a2d_sink_disconnect(connected_remote_bda);
}
